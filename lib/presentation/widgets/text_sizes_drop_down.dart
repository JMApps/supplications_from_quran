      ...
      trailing: Consumer<ContentSettingsState>(
        builder: (context, settingsState, _) {
          return DropdownButton<int>(
            ...
            value: settingsState.getTextSizeIndex,
            items: List.generate(
              AppStyles.textSizes.length,
                  (index) => DropdownMenuItem<int>(
                value: index,
                alignment: Alignment.center,
                child: Text(appLocale.textSizeNames.split(', ')[index], style: settingsState.getTextSizeIndex == index ? itemSelectedTextStyle : TextStyle(fontSize: 18.0)),
              ),
            ),
            onChanged: (newIndex) => settingsState.changeTextSizeIndex = newIndex!,
